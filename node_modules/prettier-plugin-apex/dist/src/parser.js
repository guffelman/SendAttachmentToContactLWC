...
import { findNextUncommentedCharacter, getSerializerBinDirectory, } from "./util.js";
import { APEX_TYPES, ALLOW_TRAILING_EMPTY_LINE, TRAILING_EMPTY_LINE_AFTER_LAST_NODE, } from "./constants.js";
async function parseTextWithSpawn(text, anonymous) {
...
async function parseTextWithHttp(text, serverHost, serverPort, anonymous) {
    try {
...
export default async function parse(sourceCode, options) {
    ...
    if (options.apexStandaloneParser === "built-in") {
        serializedAst = await parseTextWithHttp(sourceCode, options.apexStandaloneHost, options.apexStandalonePort, options.parser === "apex-anonymous");
    }
    else {
        serializedAst = await parseTextWithSpawn(sourceCode, options.parser === "apex-anonymous");
    }
    if (serializedAst) {
